...
#ifndef CRAZYFLIE_2_ATTITUDE_CONTROLLER_H
#define CRAZYFLIE_2_ATTITUDE_CONTROLLER_H
...
class LQRFeedforwardController {
public:
  LQRFeedforwardController();
  ~LQRFeedforwardController();

  void CalculateRotorVelocities(Eigen::Vector4d *rotor_velocities, double PWM_1,
                                double PWM_2, double PWM_3, double PWM_4);

  void SetCurrentStateFromOdometry(const EigenOdometry &odometry);
  void SetSensorData(const sensorData_t &sensors);
  void
  SetTrajectoryPoint(const mav_msgs::EigenTrajectoryPoint &command_trajectory);
  ...
  void RateController(double *delta_phi, double *delta_theta, double *delta_psi,
                      double p_command, double q_command, double r_command);
  void LQRFeedforwardControllerFunction(double *p_command, double *q_command,
  ...
  void ControlMixer(double thrust, double delta_phi, double delta_theta,
                    double delta_psi, double *PWM_1, double *PWM_2,
...
#endif // CRAZYFLIE_2_ATTITUDE_CONTROLLER_H
