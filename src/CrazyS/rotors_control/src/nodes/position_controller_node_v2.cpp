...
PositionControllerNode::PositionControllerNode() {
  ...
  geometry_pub_ = nh.advertise<mav_msgs::RateThrust>(
...
void PositionControllerNode::MultiDofJointTrajectoryCallback(
  ...
  //   ROS_INFO("Drone desired position in callback.XXX [x_d: %f, y_d: %f, z_d:
  //   %f]",
  ...
  if (n_commands >= 1) {
    ...
    ROS_INFO_ONCE(
        "PositionController got first MultiDOFJointTrajectory message.");
  }
}
...
void PositionControllerNode::writeToFile(const mav_msgs::RateThrust &msg) {
  ...
  std::ofstream file("/home/maxi/Crazyflie2-ros-gazebo-simulation/catkin_ws/"
                     "src/CrazyS/rotors_control/src/nodes/pos_contr.txt",
...
void PositionControllerNode::UpdateController() {
  ...
  if (waypointHasBeenPublished_) {
    ...
    position_controller_.HoveringController(
        &position_controller_.control_t_.thrust);
