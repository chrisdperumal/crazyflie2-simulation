...
void AttitudeControllerNode::geometryCallback(
    const mav_msgs::RateThrustConstPtr &rate_thrust) {
  last_rate_thrust_ = *rate_thrust;
}

void AttitudeControllerNode::OdometryCallback(
    const nav_msgs::OdometryConstPtr &odometry_msg) {

  // This functions allows us to put the odometry message into the odometry
  // variable--> _position,
...
void AttitudeControllerNode::UpdateController() {
  ...
  if (waypointHasBeenPublished_) {
    ...
    attitude_controller_.AttitudeControllerFunction(&p_command, &q_command,
                                                    theta_command, phi_command);

    // Compute Yaw Position Controller
    attitude_controller_.YawPositionController(&r_command);

    // Compute Rate Controller
    attitude_controller_.RateController(&delta_phi, &delta_theta, &delta_psi,
                                        p_command, q_command, r_command);

    // Compute Control Mixer
    double PWM_1, PWM_2, PWM_3, PWM_4;
    attitude_controller_.ControlMixer(attitude_controller_.control_t_.thrust,
                                      delta_phi, delta_theta, delta_psi, &PWM_1,
                                      &PWM_2, &PWM_3, &PWM_4);

    // Calculate Rotor Velocities
    attitude_controller_.CalculateRotorVelocities(&ref_rotor_velocities, PWM_1,
                                                  PWM_2, PWM_3, PWM_4);
