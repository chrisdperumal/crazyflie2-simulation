...
#include "rotors_control/position_controller_v2.h"
...
void PositionController::ControlMixer(double thrust, double delta_phi,
                                      double delta_theta, double delta_psi,
                                      double *PWM_1, double *PWM_2,
                                      double *PWM_3, double *PWM_4) {
  assert(PWM_1);
  assert(PWM_2);
  assert(PWM_3);
  assert(PWM_4);
  ...
  *PWM_4 = control_t_.thrust - (delta_theta / 2) + (delta_phi / 2) + delta_psi;
  ...
  ROS_DEBUG("Omega: %f, Delta_theta: %f, Delta_phi: %f, delta_psi: %f",
            control_t_.thrust, delta_theta, delta_phi, delta_psi);
  ROS_DEBUG("PWM1: %f, PWM2: %f, PWM3: %f, PWM4: %f", *PWM_1, *PWM_2, *PWM_3,
            *PWM_4);
}

void PositionController::CalculateRotorVelocities(
    Eigen::Vector4d *rotor_velocities, double *PWM_1, double *PWM_2,
    double *PWM_3, double *PWM_4) {
  ...
  // ROS_DEBUG("Omega_1: %f Omega_2: %f Omega_3: %f Omega_4: %f", omega_1,
  // omega_2, omega_3, omega_4);
  *rotor_velocities = Eigen::Vector4d(omega_1, omega_2, omega_3, omega_4);
}

void PositionController::RateController(double *delta_phi, double *delta_theta,
                                        double *delta_psi, double *p_command,
...
void PositionController::Quaternion2Euler(double *roll, double *pitch,
                                          double *yaw) const {
  assert(roll);
  assert(pitch);
  assert(yaw);
...
void PositionController::XYController(double *theta_command,
                                      double *phi_command) {
  ...
  *phi_command = phi_command_kp + phi_command_ki_;
  ...
  //  ROS_DEBUG("Theta_kp: %f, Theta_ki: %f", theta_command_kp,
  //  theta_command_ki_); ROS_DEBUG("Phi_kp: %f, Phi_ki: %f", phi_command_kp,
  //  phi_command_ki_); ROS_DEBUG("Phi_c: %f, Theta_c: %f", *phi_command,
  //  *theta_command); ROS_DEBUG("E_vx: %f, E_vy: %f", e_vx, e_vy);
...
void PositionController::HoveringController(double *omega) {
  ...
  if (delta_omega > MAX_POS_DELTA_OMEGA || delta_omega < MAX_NEG_DELTA_OMEGA) {
    ROS_INFO("SATURATED PHYSICAL CONSTRAINTS OF DELTA-OMEGA");
...
void PositionController::AttitudeControllerFunction(double *p_command,
                                                    double *q_command,
...
void PositionController::SetCurrentStateFromOdometry(
