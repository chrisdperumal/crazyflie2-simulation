...
void AttitudeController::AttitudeControllerFunction(double *p_command,
                                                    double *q_command,
...
void AttitudeController::XYController(double *theta_command,
                                      double *phi_command) {
...
void AttitudeController::ControlMixer(double thrust, double delta_phi,
  ...
  *PWM_4 = control_t_.thrust - (delta_theta / 2) + (delta_phi / 2) + delta_psi;
  ...
  // control_t_.thrust, delta_theta, delta_phi, delta_psi); ROS_DEBUG("PWM1: %f,
  // PWM2: %f, PWM3: %f, PWM4: %f", *PWM_1, *PWM_2, *PWM_3, *PWM_4);
}
...
void AttitudeController::RateController(double *delta_phi, double *delta_theta,
...
void AttitudeController::SetCurrentStateFromOdometry(
...
void AttitudeController::Quaternion2Euler(double *roll, double *pitch,
                                          double *yaw) const {
  assert(roll);
  assert(pitch);
  assert(yaw);
